...
const TestcaseGradingResult = ({ no,status,disableHover=false,sizeX=2,sizeY=4 }: {
	no: number | string;
	...
	return (
		<HoverCard>
			...
			{!disableHover && <HoverCardContent>
				...
				<p className="font-bold">Status: <span className={textStyledFunction()}>{status}</span></p>
...
const TestcasesGradingIndicator = ({submissionTestcases,disableHover=false,className="",sizeX=2,sizeY=4}: {
		...
		<div className={"flex gap-0.5 items-center" + className}>
			{
				submissionTestcases?.map((testcase,index) => (
					<TestcaseGradingResult sizeX={sizeX} sizeY={sizeY} disableHover={disableHover} key={index} no={index+1} status={testcase.runtime_status as TestcaseGradingResultStatus} />
				))	
			}
		</div>
	);
